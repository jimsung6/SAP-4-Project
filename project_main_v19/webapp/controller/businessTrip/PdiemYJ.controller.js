					...
					onInit : function() {
						...
						// 모델링
						 ...
						 onFilterBarSearch : function(oEvent) {
							 var that = this;
							 var PYEMP = that.getOwnerComponent().getCookiy("EMPNO");
							...
							if(RadioButtonGroup===0){
								RadioButtonGroupData="A";
							}else if( RadioButtonGroup===1 ){
								...
								console.log(EMPNO+"/"+sDateInfo+"/"+RadioButtonGroupData );
								 ...
								 this.getOwnerComponent().rfcCall("ZB_GET_PAYMENT", {   // 본인이 호출하고 싶은 RFC명 입력. 여기서는 예제로 zbsfm20_03를 사용
									...
									for(var i=0 ; i < oResultData.T_PDIEM.length ; i++){
									   ...
									   if(oResultData.T_PDIEM[i].STCODNA == '승인'){
										   oResultData.T_PDIEM[i].STCODNA = '지급요청';
									   } 
